...
def OctreeRender_trilinear_fast(rays, tensorf, chunk=4096, N_samples=-1, ndc_ray=False, white_bg=True, is_train=False, 
                                render_feature=False, out_rgb=False, out_mask=None, device='cuda'):

    rgbs, alphas, depth_maps, weights, masks = [], [], [], [], []
    features, selects = [], []
    ...
    for chunk_idx in range(N_rays_all // chunk + int(N_rays_all % chunk > 0)):
        ...
        if render_feature:
            feature_map, select_map, rgb_map, mask_map = tensorf.render_feature_map(rays_chunk, out_rgb=out_rgb, is_train=is_train, ndc_ray=ndc_ray, N_samples=N_samples)
    ...
    if render_feature: 
        ...
        if out_mask:
            # return torch.cat(features), torch.cat(masks), None, ret_rgbs
            return None, torch.cat(masks), None, ret_rgbs
        else:
            # return torch.cat(features), None, ret_rgbs
            return None, None, ret_rgbs
...
def evaluation(test_dataset,tensorf, args, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
    ...
    for idx, samples in tqdm(enumerate(test_dataset.all_rays_stack[0::img_eval_interval]), file=sys.stdout):
        ...
        np.save(f'{savePath}/{prtx}{idx:03d}_depth.npy', depth_map)
...
def evaluation_feature_text_activation_path(test_dataset,tensorf, c2ws, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
    ...
    model, _, _ = clip.create_model_and_transforms(
            "ViT-B-16",  # e.g., ViT-B-16
            pretrained="laion2b_s34b_b88k",  # e.g., laion2b_s34b_b88k
            precision="fp32",
            device=device,
        )
    tokenizer = clip.get_tokenizer("ViT-B-16")
...
def evaluation_segmentation_path(test_dataset,tensorf, c2ws, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
    ...
    classes.sort()
    text_features = model.encode_text(tokenizer(classes).to(device)).float()
    del model
