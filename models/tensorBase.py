...
def raw2alpha(sigma, dist):
    # sigma, dist  [N_rays, N_samples]positional_encoding(torch.arange(0, N_query).view(N_query, 1), 10)
...
class MLPRender_Fea_WO_View(torch.nn.Module):
    def __init__(self, inChanel, outChanel, feape=0, featureC=128):
        super().__init__()
        ...
        layer1 = torch.nn.Linear(self.in_mlpC, featureC)
        layer2 = torch.nn.Linear(featureC, featureC)
        layer3 = torch.nn.Linear(featureC, outChanel)

        # self.mlp = torch.nn.Sequential(layer1, layer2, layer3)
        self.mlp = torch.nn.Sequential(layer1, torch.nn.ReLU(), layer2, torch.nn.ReLU(), layer3)
